...
exports.userStatus = exports.getUsersByRole = exports.fetchUsers = exports.adminLogin = void 0;
...
const adminLogin = async (req, res) => {
    try {
        ...
        // 4. Generate JWT
        const token = jsonwebtoken_1.default.sign({ adminId: admin._id, role: admin.role }, process.env.JWT_SECRET, { expiresIn: "1d" });
...
const getUsersByRole = async (req, res) => {
    try {
        ...
        if (role === "owner") {
            ...
            const owners = await ownerModel_1.default.find()
            ...
            const tenants = await userModel_1.default.find({ role: "tenant" }).select("-password -refreshToken").lean();
            return res.status(200).json({
                success: true,
                role,
            ...
            const admins = await adminModel_1.Admin.find().select("-password -refreshToken").lean();
            return res.status(200).json({
                success: true,
                role,
                count: admins.length,
...
const userStatus = async (req, res) => {
    try {
        ...
        const user = await userModel_1.default.findById(cleanId);
        if (!user) {
